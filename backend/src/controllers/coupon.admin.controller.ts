...
// GET /api/coupons/public — active coupons for checkout page display
export const getActiveCoupons = asyncHandler(
  async (req: Request, res: Response) => {
    const coupons = await Coupon.find({
      isActive: true,
      $or: [{ expiresAt: { $gt: new Date() } }, { expiresAt: null }],
    }).select("code discountType discountValue minimumOrder expiresAt");
    res.status(200).json(new ApiResponse(200, "Active coupons", coupons));
  },
);
