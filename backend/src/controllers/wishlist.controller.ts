...
export const moveWishlistToCart = asyncHandler(async (req: Request, res: Response) => {
  const wishlist = await moveWishlistToCartService(req.user!._id);
  res.status(200).json(new ApiResponse(200, "Wishlist items moved to cart successfully", wishlist));
});

// PATCH /api/wishlist/share
export const toggleWishlistShare = asyncHandler(async (req: Request, res: Response) => {
  const { shareEnabled } = req.body;
  const wishlist = await toggleWishlistShareService(req.user!._id, shareEnabled);
  res.status(200).json(new ApiResponse(200, `Wishlist sharing ${shareEnabled ? "enabled" : "disabled"}`, wishlist));
});
...
export const getSharedWishlist = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.status(200).json(new ApiResponse(200, "Shared wishlist fetched successfully", wishlist));
});

// POST /api/wishlist/email
export const emailWishlist = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!wishlist.shareEnabled || !wishlist.shareId) {
    throw new ApiError(400, "Sharing must be enabled before you can email your wishlist");
  }
